...
export function startRandomEventTriggerLoop(initialDelay = 1000) {
  const triggerNextEvent = () => {
    ...
    const nextDelay = Math.floor(Math.random() * 30000) + 15000; // 다음 이벤트까지 15~45초
    eventTimerId = setTimeout(triggerNextEvent, nextDelay);
  };

  eventTimerId = setTimeout(triggerNextEvent, initialDelay);
}

export function stopRandomEventTriggerLoop() {
  if (eventTimerId) {
